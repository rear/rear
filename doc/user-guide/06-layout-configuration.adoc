...
----------------------------------
keyword vaule1 value2 ...
----------------------------------
where keyword denotes one kind of component (dik, partition, filesystem, ...)
...
Normal text has to be present verbatim in the file. Angle
...
All other lines start with a component keyword. None of the component keywords
is a leading substring of another component keyword (e.g. disk is not a
leading substring of opaldisk) so that one can get the lines that
belong to a particular component via simple commands like
----------------------------------
...
The +/var/lib/rear/layout/disklayout.conf+ file is being used as input during +rear recover+ to create on-the-fly a script called +/var/lib/rear/layout/diskrestore.sh+.
