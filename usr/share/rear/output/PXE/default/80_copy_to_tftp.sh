# #80_copy_to_tftp.sh
#
# copy kernel and initrd to TFTP server for Relax & Recover
#
...
cp -pL $v "$KERNEL_FILE" "$PXE_TFTP_LOCAL_PATH/$PXE_KERNEL"
cp -a $v "$BUILD_DIR"/initrd.cgz "$PXE_TFTP_LOCAL_PATH/$PXE_INITRD"
