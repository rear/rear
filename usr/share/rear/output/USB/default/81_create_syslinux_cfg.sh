...
for file in $(cd $BUILD_DIR/netfs; ls rear/*/????????.????/syslinux.cfg); do
    ...
    echo "    include /$file" >&4
    oldsystem=$system
done 4>$USB_SYSLINUX_DIR/rear.cfg

if [[ "$oldsystem" ]]; then
    # Close last submenu
    echo "menu end" >>$USB_SYSLINUX_DIR/rear.cfg
fi
