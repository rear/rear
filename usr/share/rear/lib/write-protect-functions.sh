...
# Only the is_write_protected function is called by other scripts.

function is_write_protected() {
    local candidate_device="$1"
    # returns 0 if the device node of the candidate device
    # is designated as write-protected by any of the below means.

    # Local helper functions for the is_write_protected function:
    ...
    function write_protected_candidate_device() {
        ...
        # see https://github.com/google/cadvisor/issues/3340
        # and https://github.com/rear/rear/issues/3085
        if test -e "$device" ; then
            test -b "$device" || BugError "write_protected_candidate_device called for '$1' but '$device' is no block device"
            echo "$device"
            return 0
        fi
        DebugPrint "write_protected_candidate_device called for '$1' but '$device' does not exist"
        return 1
    }
    ...
    function is_write_protected_by_fs_label() {
        local device="$( write_protected_candidate_device "$1" )"
        # returns 0 if one of the device's file system labels
    ...
    # End of local helper functions for the is_write_protected function.

    local device=''
    # write_protected_candidate_device returns 1
    # when the candidate device does not have a matching "dev" file
    # e.g. /sys/block/nvme0c0n1 does not have a /dev/nvme0c0n1 device node
    # see https://github.com/rear/rear/issues/3085
    # and because this is meant to identify write-protected disks and partitions
    # only candidate devices with a device node are considered for write protection
    # so candidate devices without device node get reported as not write protected:
    if ! device="$( write_protected_candidate_device "$candidate_device" )" ; then
        Log "$candidate_device cannot be write-protected because there is no '/dev/...' device node"
        return 1
    fi
