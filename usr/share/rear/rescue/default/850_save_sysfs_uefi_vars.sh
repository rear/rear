...
if test -f "$UEFI_BOOTLOADER" ; then
    # Show to the user a confirmation when his specified UEFI_BOOTLOADER will be used:
    ...
    for find_name_pattern  in "${UEFI_BOOTLOADER[@]}" ; do
        ...
        test -f "$UEFI_BOOTLOADER" && break || UEFI_BOOTLOADER=$( find /boot -iname "$find_name_pattern" | tail -1 )
    done

    if test -f "$UEFI_BOOTLOADER" ; then
        # Show to the user what was actually found for his UEFI_BOOTLOADER 'find' pattern(s):
        LogPrint "Found '$UEFI_BOOTLOADER' as UEFI bootloader file"
    else

        # When UEFI_BOOTLOADER is not specified or when it does not exist or is no regular file
        # try to find "well known files" to be used as UEFI bootloader file:
        LogPrint "Trying to find a 'well known file' to be used as UEFI bootloader file"
        ...
        # If no valid UEFI_BOOTLOADER was autodetected, try more generic finds in whole /boot with case insensitive filename matching.
        ...
        if test -f "$UEFI_BOOTLOADER" ; then
            # Show to the user which 'well known file' was actually found:
            LogPrint "Found '$UEFI_BOOTLOADER' to be used as UEFI bootloader file"
        else

            # When UEFI_BOOTLOADER is not specified or when it does not exist or is no regular file
            # try to autodetect from EFI variables what to use as UEFI_BOOTLOADER:
            LogPrint "Autodetecting from EFI variables what to use as UEFI bootloader file"
            ...
            if test -f "$UEFI_BOOTLOADER" ; then
                # Show to the user what was actually autodetected:
                LogPrint "Autodetected from EFI variables '$UEFI_BOOTLOADER' to be used as UEFI bootloader file"
            else
