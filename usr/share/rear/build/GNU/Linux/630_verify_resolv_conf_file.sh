...
# When USE_DHCLIENT has a true value and USE_STATIC_NETWORKING does not have a true value
# the etc/resolv.conf in the recovery system is generated by /bin/dhclient-script
# cf. https://github.com/rear/rear/pull/2018#issuecomment-470968951
# so that the content of /etc/resolv.conf does not matter and we do not need to error out
